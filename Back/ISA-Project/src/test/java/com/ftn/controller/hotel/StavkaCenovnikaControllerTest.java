...
@SpringBootTest
public class StavkaCenovnikaControllerTest {
...
private static final String URL_PREFIX = "/api/price";
	...
	public void testGetAllPricesInPricelist() throws Exception {
		mockMvc.perform(get(URL_PREFIX + "/getAllPrices/10" )).andExpect(status().isOk())
	...
	public void testCreatePrice() throws Exception {
		StavkaCenovnikaHotelaDTO stavka = new StavkaCenovnikaHotelaDTO();
		stavka.setCena(Double.toString(2500));
		stavka.setTipStavke("JEDNOKREVETNA_SOBA");
		String json = com.ftn.utils.TestUtil.json(stavka);
		this.mockMvc.perform(post(URL_PREFIX + "/createPrice/11").contentType(contentType).content(json)).andExpect(status().isOk());
	}
